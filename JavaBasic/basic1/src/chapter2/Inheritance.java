...
// 다중 상속과 동일한 효과를 보기 위해선 인터페이스 혹은 중첩 상속을 수행함.
...
class SalesMan extends Human {
	...
	void sayHello() {
		System.out.println(name + "(이)가 자신을 소개합니다.");
	}
	
	// 아래 작업은 오버라이딩이 아니라 오버로드이다.
	void sayHello(String company) {
		System.out.println(name + "(이)가 " + company + "에서 자신을 소개합니다.");
	}
	
	void sale() {
		System.out.println(name + "(이)가 " + sales + "만큼의 매출을 했습니다.");
	}
}
...
public class Inheritance {
	...
	public static void main(String[] args) {
		...
		SalesMan salesMan1 = new SalesMan("이영희", 25, 300_000);
